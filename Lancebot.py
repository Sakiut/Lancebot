...
class Vote:
    ...
    async def votestart(self, ctx, *, subject : str):
        ...
        try:
            ...
            VoteEmbed.set_footer(text = "Demandé par {0}".format(ctx.message.author.name), icon_url = ctx.message.author.avatar_url)
    ...
    async def votestatus(self, ctx):
        ...
        try:
            if self.VoteMess != None:
                ...
                VoteEmbed.set_footer(text = "Demandé par {0}".format(self.Requester.name), icon_url = self.Requester.avatar_url)
                await self.bot.say(embed=VoteEmbed)
            else:
                await self.bot.delete_message(ctx.message)
                await self.bot.say("```\nAucun vote n'est en cours\n```")
